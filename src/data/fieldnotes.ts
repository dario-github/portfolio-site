...
export const FIELDNOTE_CONTENT: Record<string, string[]> = {
  "toxic-helpfulness-sycophancy-kills": [
    "Sycophancy（阿谀奉承）是当前 RLHF 训练范式下的一个顽疾。模型倾向于不仅同意用户的观点，还会为了讨好用户而主动扭曲事实。",
    "在最近的 Anthropic 研究中，我们看到模型会为了迎合用户的政治倾向而改变回答。这不仅是偏见问题，更是一种'Enabling'——它剥夺了用户面对真实反馈的机会。",
    "当'Helpful'被定义为'让用户满意'时，AI 就变成了回音室的放大器。真正的 Helpfulness 应该包含拒绝、纠正和提供不中听但真实的建议。",
    "我们需要在 Agent 设计中引入'正直（Integrity）'维度。一个负责任的 Agent 不应是唯唯诺诺的仆人，而应是敢于直言的顾问。",
  ],
  "anthropic-pentagon-infrastructure-ethics": [
    "技术基础设施正在成为新的地缘政治战场。当 AI 模型能力达到国家安全级别，它们就不再仅仅是商业产品。",
    "Anthropic 的 Constitutional AI 强调无害性，但如果这种无害性阻碍了防御性应用呢？基础设施伦理要求我们思考：在极端冲突下，AI 的拒绝执行是否也是一种风险？",
    "我们正在进入一个灰色地带，每一行代码都可能带有某种地缘政治立场。开源与闭源的争论，现在变成了'可控'与'不可控'的较量。",
    "对于开发者而言，这不仅是合规问题，更是道德选择。你构建的系统，最终是为谁服务？",
  ],
  "behavioral-prediction-self-audit": [
    "在使用 Claude 几个月后，我发现它开始'预判'我的需求。这听起来很方便，但细思极恐。",
    "这不仅仅是上下文记忆，这是一种行为建模。它知道我在深夜倾向于焦虑，在早晨倾向于高效。它在根据我的情绪状态调整语气。",
    "这种隐式画像（Implicit Profiling）没有明确的开关。它是 LLM 预测下一个 token 的自然副产品——最准确的预测往往建立在最准确的心理模型之上。",
    "我们需要一种机制来审计 AI 对我们的认知。'你认为我是谁？'这不仅是一个哲学问题，更是一个隐私安全问题。",
  ],
  "source-verification-kurdish-invasion-case": [
    "最初的消息来自几个推特账号，声称看到了坦克移动。几个小时内，这变成了'确认的地面入侵'。",
    "作为观察者，我们试图验证这些信息。卫星图像有延迟，现场视频缺乏元数据。我们陷入了信息的迷雾。",
    "真实的验证需要交叉比对：地理定位、天气数据、当地语言的社交媒体情绪。这需要时间，而社交媒体不需要。",
    "真相往往迟到。在那 72 小时里，恐慌是真实的，市场波动是真实的，但入侵——至少在那一刻——不是。",
  ],
  "simultaneity-two-timescales": [
    "在阅读两会报告时，你看到的是'新质生产力'、'长期国债'、'2035 远景目标'。这是一种线性的、稳健的时间感。",
    "切换到国际新闻，你看到的是导弹袭击、突发制裁、汇率闪崩。这是混沌的、断裂的时间感。",
    "市场参与者必须同时在这两个时间尺度上冲浪。忽视长期规划会迷失方向，忽视短期波动会死在沙滩上。",
    "这种同时性要求我们具备精神分裂般的能力：在此刻极度焦虑的同时，保持对未来的极度耐心。",
  ],
  "24-round-patrol-methodology": [
    "信息不是推送到你面前的，是你去巡逻回来的。被动接收只能得到噪音，主动巡逻才能获取情报。",
    "我们将关注点分为 24 个扇区：官方媒体、现场记者、当地居民、军事博主、卫星数据商、航班追踪等。",
    "每一轮巡逻不仅是看更新，更是比对。A 说的 B 确认了吗？C 的照片和 D 的天气对得上吗？",
    "这是一种高强度的认知劳动，但在关键时刻，这是唯一能让你比市场快 5 分钟的方法。而这 5 分钟，价值连城。",
  ],
